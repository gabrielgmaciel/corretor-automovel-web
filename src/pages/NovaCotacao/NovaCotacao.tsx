...
export default function NovaCotacao() {
    ...
    const [importandoApolice, setImportandoApolice] =
        useState(false);

    const [resultadoApolice, setResultadoApolice] =
        useState<string[]>([]);
    ...
    const encontrarDominioPorDescricao = <T extends {
        descricao: string;
    }>(
        itens: T[],
        descricao: string
    ...
    const importarApolice = async (arquivo?: File) => {
        if (!arquivo) return;
        ...
        try {
            ...
            const response = await api.post<ApoliceExtraida>(
                "/assistente/apolice",
                formulario,
                {
                    headers: {
            ...
            if (dados.segurado?.nome) setNome(dados.segurado.nome);
            if (dados.segurado?.email) setEmail(dados.segurado.email);
            ...
            if (dados.endereco) {
                ...
                if (dados.endereco.numero) setNumero(dados.endereco.numero);
                ...
                setEndereco(current => ({
                    ...current,
                    logradouro:
                        dados.endereco?.logradouro || current.logradouro,
            ...
            if (
                ...
                let veiculosEncontrados: Veiculo[] = [];

                if (dados.veiculo.codigoFipe) {
                    try {
                        const responseFipe = await api.get<Veiculo[]>(
                            "/automovel/buscar/fipe",
                            {
                                params: {
                                    codigoFipe:
                                        dados.veiculo.codigoFipe
                        ...
                        avisos.push(
                            `Não foi possível buscar pelo código FIPE ${dados.veiculo.codigoFipe}. A busca continuará pelo modelo.`
                        );
                    }
                }
                ...
                if (
                    ...
                    const responseModelo = await api.get<Veiculo[]>(
                ...
                const modeloApolice =
                    normalizarTexto(dados.veiculo.modelo || "");
                const veiculo = modeloApolice
                    ? veiculosEncontrados.find(item =>
                        normalizarTexto(item.modelo).includes(modeloApolice)
                ...
                if (veiculo) {
                    ...
                    if (dados.veiculo.anoFabricacao) {
                        ...
                        if (veiculo.anoFabricacao.includes(ano)) {
                            ...
                            avisos.push(
                                `Ano de fabricação ${ano} não disponível para o veículo encontrado.`
                            );
                        }
                    }
                    if (dados.veiculo.anoModelo) {
                        ...
                        if (veiculo.anoModelo.includes(ano)) {
                            ...
                            avisos.push(
                                `Ano modelo ${ano} não disponível para o veículo encontrado.`
                            );
                        }
                    }
                    ...
                    const identificacao =
                        dados.veiculo.codigoFipe
                            ...
                            : descricao;
                    avisos.push(
                        `Veículo não encontrado no catálogo: ${identificacao}.`
                    );
                }
            }

            if (dados.franquia?.descricao) {
                const franquia = encontrarDominioPorDescricao(
                    franquias,
                    dados.franquia.descricao
                );
                if (franquia) {
                    setFranquiaSelecionada(franquia.codigo);
                } else {
                    avisos.push(
                        `Franquia não associada: ${dados.franquia.descricao}.`
                    );
                }
            }

            const adicionais: Cobertura[] = [];
            const acessoriosImportados: Cobertura[] = [];
            const valoresPrincipais: Record<string, string> = {};
            const valoresProtecaoImportados: Record<string, string> = {};

            for (const item of dados.coberturas || []) {
                if (!item.descricao) continue;
                const cobertura = encontrarDominioPorDescricao(
                    coberturas,
                    item.descricao
                );
                if (!cobertura) {
                    avisos.push(
                        `Cobertura não associada: ${item.descricao}.`
                    );
                    continue;
                }
                ...
                if (
                    ...
                    if (item.valor != null) {
                        valoresPrincipais[cobertura.codigo] =
                            formatarMoedaInput(
                                String(Math.round(item.valor * 100))
                            );
                    }
                } else if (
                    cobertura.tipoCobertura === "coberturas_adicionais"
            ...
            setErro(
                "Não foi possível ler a apólice. Verifique o PDF e tente novamente."
            );
        } finally {
    ...
    return (
        <div className={styles.page}>
            <div className={styles.container}>
                ...
                <div className={styles.policyImport}>
                    <div>
                        <strong>Importar apólice em PDF</strong>
                        <span>
                            A IA lê o documento e preenche os campos encontrados.
                            Revise todos os dados antes de realizar a cotação.
                        </span>
                    </div>

                    <label className={styles.policyImportButton}>
                        {importandoApolice
                            ? "Lendo apólice..."
                            : "Selecionar PDF"}
                        <input
                ...
                {resultadoApolice.length > 0 && (
                    <div className={styles.policyImportResult}>
                        {resultadoApolice.map(aviso => (
